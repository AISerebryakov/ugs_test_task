...
func TestRepository_SelectQuery(t *testing.T) {
	var testCases = []struct {
		...
		{name: "ById", byId: "test_id", ascendingExists: true, ascendingValue: true, limit: 20, offset: 4,
			resultSql:  "SELECT id, create_at, address, location FROM buildings WHERE id = $1 ORDER BY create_at ASC LIMIT 20 OFFSET 4",
		...
		{name: "ByIdAndFromDate", byId: "test_id", ascendingExists: true, ascendingValue: true, fromDate: 7000, limit: 20,
		...
		{name: "ByIdAndToDate", byId: "test_id", ascendingExists: true, ascendingValue: true, toDate: 9000, limit: 20, offset: 4,
			resultSql:  "SELECT id, create_at, address, location FROM buildings WHERE id = $1 AND create_at <= $2 ORDER BY create_at ASC LIMIT 20 OFFSET 4",
			resultArgs: []interface{}{"test_id", int64(9000)}},
		{name: "ByIdAndFromAndToDate", byId: "test_id", ascendingExists: true, ascendingValue: true, fromDate: 7000, toDate: 9000, limit: 20,
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			query := newSelectQuery(context.Background()).
				ById(tc.byId).ByAddress(tc.byAddress).
